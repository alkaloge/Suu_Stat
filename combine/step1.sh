# create root files with proper dir

var=${1}
var="recoSuu_mass0"
cut="300"
#python3 SetupSuu.py --year 2017 --cut 500
#python3 SetupSuu.py --year 2018 --cut 500
python3 SetupSuu.py --year 2018 --cut ${cut} --var ${var}
python3 SetupSuu.py --year 2017 --cut ${cut} --var ${var}
python3 SetupSuu.py --year 2016 --cut ${cut} --var ${var}
python3 SetupSuu.py --year 2016APV --cut ${cut} --var ${var}


hadd -f ULRun2/step1_Run2_recoSuu_mass0_JetpTgt300.root UL2018/step1*recoSuu_mass0_JetpTgt300.root UL2017/step1*recoSuu_mass0_JetpTgt300.root UL2016/step1*recoSuu_mass0_JetpTgt300.root  UL2016APV/step1*recoSuu_mass0_JetpTgt300.root

#mkdir UL2016
#hadd  -f UL2016/step1_2016_${var}_JetpTgt${cut}.root ULpostVFP_2016/step1_postVFP_2016_${var}_JetpTgt${cut}.root ULpreVFP_2016/step1_preVFP_2016_${var}_JetpTgt${cut}.root

